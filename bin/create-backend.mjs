...
function packageJsonContent(projectName, framework) {
...
function dockerfileContent(framework) {
    if (framework === "node") {
...
function postInstallCheck(projectPath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
...
`);
        ...
        const selectedFramework = FRAMEWORK_CHOICES.find((f) => f.value === response.framework);
        ...
        if (response.framework === "node") {
            ...
            try {
                execSync(`npx @nestjs/cli new ${response.projectName} --skip-git --package-manager npm`, {
        ...
        createFileWithContent(path.join(rootPath, "package.json"), packageJsonContent(response.projectName, response.framework));
        ...
        createFileWithContent(path.join(rootPath, "Dockerfile"), dockerfileContent(response.framework));
        ...
        console.log(`\n✅ Project "${response.projectName}" created successfully with ${selectedFramework === null || selectedFramework === void 0 ? void 0 : selectedFramework.title} and ${response.structure} structure!`);
        // Installation logic
        try {
            const { installDeps } = yield prompts({
                type: "confirm",
                name: "installDeps",
            ...
            if (installDeps) {
                yield installDependencies(rootPath);
                yield postInstallCheck(rootPath);
                const { fixIssues } = yield prompts({
                    type: "confirm",
                    name: "fixIssues",
